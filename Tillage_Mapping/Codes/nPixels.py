...
def addIndices(image):
  ...
  return image.addBands(evi).addBands(gcvi)  .addBands(ndvi).addBands(sndvi).addBands(ndti).  addBands(ndi5).addBands(ndi7).addBands(crc).addBands(sti)
...
GridMet = ee.ImageCollection("IDAHO_EPSCOR/GRIDMET")                                  .filter(ee.Filter.date('2021-1-1','2022-12-30'))                                  .filterBounds(geometry);
def MoistMask(img):
...
def makeComposite (year, orgCollection):
    ...
    composite2 = orgCollection      .filterDate(
    ...
    return ee.ImageCollection(composite1)      .merge(ee.ImageCollection(composite2))
...
# This groups images and rename bands
  ...
  collection_2 = orgCollection    .filterDate(
      ee.Date.fromYMD(ee.Number(year).add(ee.Number(1)), 3, 1),
      ee.Date.fromYMD(ee.Number(year).add(ee.Number(1)), 5, 30)
    ).filterBounds(geometry)\
    .map(addDOY)\
...
L8_2122 = L8T1  .filter(ee.Filter.calendarRange(startYear, endYear, 'year'))  .map(lambda img: img.set('year', img.date().get('year')))  .map(lambda img: img.clip(geometry))

L7_2122 = L7T1  .filter(ee.Filter.calendarRange(startYear, endYear, 'year'))  .map(lambda img: img.set('year', img.date().get('year')))  .map(lambda img: img.clip(geometry))
