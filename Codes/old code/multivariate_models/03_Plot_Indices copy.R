...
mixed_reflect_df <- read.csv(paste0(path_to_data, 
                                  "mixed_reflect_df.csv"),
                           header = TRUE, row.names = NULL)

mixed_reflect_df <- mixed_reflect_df[, -c(2, 3, 4, 5)]

mixed_reflect_df <- mixed_reflect_df[mixed_reflect_df$Wvl > 1400, ]


colnames(mixed_reflect_df)[4] <- "Reflectance"
colnames(mixed_reflect_df)[5] <- "RWC"
...
# Now use dcast to spread data using the new interaction term
CAI <- dcast(CAI, ... ~ Wvl, value.var = "Reflectance", fill = 0)

CAI[, `:=`(CAI = 0, SINDRI = 0, NDTI = 0)]
...
CAI$R2260_2280 <-  rowMeans(select_columns_range(CAI, '2290', '2330'))
...
desired_columns <- c("Mix", "Fr", "RWC", "CAI", "SINDRI", "NDTI")
CAI<- CAI[, ..desired_columns]

write.csv(CAI, file = paste0(path_to_data, "Mixed_indices_df.csv"), row.names = FALSE)
...
#     print(combined_plot)
#     
#     ggsave(filename = paste0(path_to_plots, "RWC_Reflectance/", soil,"_", crp, ".png"), plot = combined_plot, width = 5, height = 8, dpi = 300)
#   }
# }
# 
