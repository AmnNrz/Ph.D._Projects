...
path_to_save <- paste0('/Users/aminnorouzi/Library/CloudStorage/',
                       'OneDrive-WashingtonStateUniversity(email.wsu.edu)/',
                       'Ph.D/Projects/Soil_Residue_Spectroscopy/Data/',
                       'multivariate_model/index_crp_sl/')
...
for (crp in sort(crops)) {
  for (sl in sort(soils)) {
    ...
    write.csv(test3_NDTI, paste0(path_to_save, "NDTI_", crp, "_", gsub("_", " ", sl), ".csv"), row.names = FALSE)
    ...
    write.csv(test3_CAI, paste0(path_to_save, "CAI_" , crp, "_", gsub("_", " ", sl), ".csv"), row.names = FALSE)
    ...
    write.csv(test3_SINDRI, paste0(path_to_save, "SINDRI_", crp, "_", gsub("_", " ", sl),  ".csv"), row.names = FALSE)
