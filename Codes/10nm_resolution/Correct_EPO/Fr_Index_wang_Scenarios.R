...
Xsr_combined_indices <- read.csv(paste0(path_to_data,
                                        "Xsr_Original_Transformed_indices_allCrop_allSoils.csv"))
...
all_rwc_levels <- unique(c(Xsr_combined_indices$RWC))
...
unique_rwc <- unique(Xsr_combined_indices$RWC)
...
subset_df_long <- Xsr_combined_indices %>% 
...
p <- ggplot(subset_df_long, aes(x = Index_val, y =Fraction , color = factor(round(RWC, digits = 2)),
                               shape = factor(RWC))) +
...
p2 <- ggplot(subset_df_long, aes(x = Index_val, y = Fraction, color = factor(round(RWC, digits = 2)),
                                 shape = factor(RWC))) +
  geom_smooth(method = "lm", se = FALSE, aes(group = factor(RWC))) +
  labs(
    # title = paste0(subset_df_long$Mix[1]),
...
p2 <- ggplot(subset_df_long, aes(x = Index_val, y = Fraction, color = factor(round(RWC, digits = 2)))) +
  ...
  labs(
    # title = paste0(subset_df_long$Mix[1]),
    x = "Index", y = "Residue Cover Fraction",
    color = "RWC" # Legend title for color
  ) +
  theme_minimal() +
  theme(
...
SINDRI <- dplyr::filter(subset_df_long, Index == "SINDRI" & source == "EPO")
SINDRI <- dplyr::filter(SINDRI, Index_val>= -1 & Index_val <= 1)
p2 <- ggplot(SINDRI, aes(x = Index_val, y = Fraction, color = factor(round(RWC, digits = 2)))) +
  geom_point() + # Add actual data points
  labs(
    # title = paste0(subset_df_long$Mix[1]),
    x = "Index", y = "Residue Cover Fraction",
