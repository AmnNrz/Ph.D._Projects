...
P <- read.csv(paste0(path_to_data, "P_fresh_dark.csv"), check.names = FALSE)
...
for (mix in unique(mixed_original$Type)){
  combined_df_fr <- data.frame()
  for (fr in unique(mixed_original$Fraction)){
    mixed_original_filtered_mix <- dplyr::filter(mixed_original, Type == mix)
    mixed_original_filtered_fr <- dplyr::filter(mixed_original_filtered_mix, Fraction == fr)
    
    rwc <- unique(mixed_original_filtered_fr$crop_rwc)[1]
    Xsr_hat <- data.frame()
    for (rwc in unique(mixed_original_filtered_fr$crop_rwc)){
      mixed_original_filtered_rwc <- dplyr::filter(mixed_original_filtered_fr, crop_rwc == rwc)
      Xsr <- mixed_original_filtered_rwc$Reflect
      Xsr <- as.matrix(Xsr)
      
      Xsr_hat_rwc <- t(Xsr) %*% as.matrix(P)
      Xsr_hat_rwc <- as.data.frame(t(Xsr_hat_rwc))
      mixed_original_filtered_rwc$Reflect_val <- Xsr_hat_rwc[, 1, drop = TRUE]
      Xsr_hat <- rbind(Xsr_hat, mixed_original_filtered_rwc)
    }

    combined_df_fr <- rbind(combined_df_fr, Xsr_hat)
      # rownames(Xsr_hat) <- mixed_original_filtered$Wvl
  }
  combined_df <- rbind(combined_df, combined_df_fr)
  }
