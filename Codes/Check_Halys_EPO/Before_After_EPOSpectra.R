...
Crop_original_epo <- read.csv(paste0(path_to_data, "Residue.csv"))
Crop_original <- Crop_original_epo[-c(1, 8)]

Crop_original$Sample <- gsub("Crop Residue", "Residue", Crop_original$Sample)
...
Crop_EPO <- Crop_original_epo[-c(1, 7)]
Crop_EPO$Source <- 'EPO' # Add a source column

Crop_EPO <- dplyr::rename(Crop_EPO, Type_Name = Soil)
Crop_EPO <- dplyr::rename(Crop_EPO, Reflect = EPO_Reflect)
...
for (crp in unique(Crop_combined$Type_Name)){
  ...
  p <- ggplot(df, aes(Wvl, Reflect, group = factor(RWC))) +
    ...
    geom_vline(xintercept = c(2000, 2100, 2200), linetype = "solid", size = 0.6, color = 'red') +  # Add vertical lines
    geom_vline(xintercept = c(1660, 2330), linetype = "solid", size = 0.6, color = 'blue') +  # Add vertical lines
    geom_vline(xintercept = c(2200, 2260), linetype = "solid", size = 0.6, color = 'green') +  # Add vertical lines
    ...
    scale_y_continuous(labels = label_number(accuracy = 0.01), breaks = y_breaks) +  # custom y-axis grid lines
    scale_color_manual(values = custom_colors,
                       name = "RWC levels") +  # Add legend title here
    theme_minimal() +
    theme(
  ...
  ggsave(paste0(path_to_plots, "Before_after_EPO_spectra/", crp, ".png"), combined_plot,
         width = 45, height = 15, units = "cm",dpi = 200)
  # 
}
