library(magrittr)    # If you just installed magrittr
library(dplyr)       # If you installed dplyr
library(tidyverse)   # If you installed tidyverse
...
PeasResidue <- Residue_Median %>%
                 filter(Type_Name=="Peas") %>% select(-starts_with("RWC")) %>%
                   pivot_wider(names_from = "Scan", values_from  = "Reflect")
