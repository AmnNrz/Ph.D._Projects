...
CropList <- unique(Residue_Median$Type_Name)
SoilList <- unique(Soil_Median$Type_Name)
LinearMix <- data.frame()
for (i in (CropList)) {
  Cropi_i <- dplyr::filter(Residue_Median, Type_Name == i)
  names(Cropi_i)[6] <- "RWC_Residue"
  names(Cropi_i)[5] <- "Ref_Residue"
  TempCrop <- Cropi_i
  for (j in (SoilList)) {
    TempSoil <- data.frame()
    Soil_j <- dplyr::filter(Soil_Median, Type_Name == j)
