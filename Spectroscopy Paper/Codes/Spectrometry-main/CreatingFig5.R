...
setwd("G:\\My Drive\\PhD\\Projects\\Spectroscopy paper\\Spectrometry-main")

Samples <- read.csv("G:\\My Drive\\PhD\\Projects\\Spectroscopy paper\\Spectrometry-main\\Samples.csv")
WV_Group <- Samples
...
print(length(unique(WV_Group$Scan)))
print(unique(WV_Group$Crop))
...
for (i in length(unique(WV_Group$Scan))) {
  ...
  for (j in unique(a$Crop)) {
    c <- dplyr::filter(dplyr::filter(a, Crop == j))
