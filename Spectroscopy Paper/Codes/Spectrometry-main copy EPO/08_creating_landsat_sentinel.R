...
CAI <- read_csv("/Users/aminnorouzi/Library/CloudStorage/GoogleDrive-msaminnorouzi@gmail.com/My Drive/PhD/Projects/Spectroscopy paper/Spectrometry-main/CAI_Combined.csv")
...
Samples %>%
  ...
  group_by(Scan,Crop,Sample,Bands)%>%
  dplyr::summarise(Reflect = median(Reflect),
                   RWC = median(RWC))%>%
  pivot_wider(names_from = Bands,values_from = Reflect)%>%
...
write.csv(Samples, file = "Samples.csv", row.names = FALSE)
