...
setwd(paste0("/home/amnnrz/GoogleDrive - msaminnorouzi/PhD/Projects/",
...
df_original$Slope <- df_original_minRWC$Slope[1]
df_original$Intercept <- df_original_minRWC$Intercept[1]
df_original$Fr_est <- df_original$Intercept + 
  df_original$Slope * df_original$CAI
...
df_original <- data.frame()
for (sl in soils){
  index_df <- read.csv(paste0(path_to_directory, 'estAFTER_EPO/CAI_transformed_', CROP, '_', paste0(sl), '.csv'))
  index_df$soil <- sl
  index_df$crop <- CROP
  df_original <- rbind(df_original, index_df)
