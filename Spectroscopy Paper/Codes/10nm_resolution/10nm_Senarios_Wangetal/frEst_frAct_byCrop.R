...
crp <- unique(Xsr_combined_indices$Crop)[4]
...
for (crp in unique(Xsr_combined_indices$Crop)){
  ...
  mix <- unique(subset_df$Mix)[1]
  
  for (mix in unique(subset_df$Mix)){
    print(paste(crp, mix))
    mix_df <- dplyr::filter(subset_df, Mix == mix)
    ...
    dry_df_EPO <- dplyr::filter(mix_df, source == "EPO")
    ...
    ggsave(filename = paste0(path_to_plot, "frEst_Act/byCrop/", index, "/", index, "_", mix, ".png"), plot = p_combined, dpi = 200, width = 14, height = 5, units = "in")
  }
}
