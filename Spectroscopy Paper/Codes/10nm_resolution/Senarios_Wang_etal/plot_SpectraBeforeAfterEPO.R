...
for (mix in unique(Xsr_combined$Mix)){
  ...
  plot <- ggplot(Xsr_mix, aes(x = Wvl, y = Reflect, group = RWC)) + 
  ...
  ggsave(filename = paste0(path_to_plot, "BeforeAfterEPO_Reflect/", mix, ".png"), plot = plot, dpi = 200, width = 10, height = 3.5, units = "in")
}
