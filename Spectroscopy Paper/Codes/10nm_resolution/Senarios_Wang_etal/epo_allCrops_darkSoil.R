...
######  Filter for dark soil  
mixed_original <- 
  mixed_original[grepl("_(Bagdad|Mondovi 1|Athena)$", mixed_original$Type), ]
...
######  Filter for dark soil  
Soil <- dplyr::filter(Soil, Soil$Type %in% c('Bagdad', 'Mondovi 1', 'Athena'))
...
write.csv(Xsr_transformed, file = paste0(path_to_data, "allCrops_darkSoil/Xsr_Transformed.csv"),
          row.names = FALSE)
...
write.csv(mixed_original, file = paste0(path_to_data, "allCrops_darkSoil/Xsr_Original.csv"),
          row.names = FALSE)
