...
for r in residue_df['Type_Name'].unique():
    ...
    # Add Sample row
    Sample = pd.DataFrame({'Sample': [res_df['Sample'].iloc[0]] * d.shape[1]}).T
...
df_transformed = pd.melt(X_transformed_copy, id_vars=['Wvl'], var_name='RWC', value_name='Reflect')
df_transformed
...
df_transformed.to_csv(path_to_data + "Residue_Transformed_Dmerged.csv", index=False)
