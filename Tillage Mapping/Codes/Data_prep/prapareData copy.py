...
colnames = ['pointID', 'NDVI', 'EVI', 'blue', 'green', 'red', 'ni', 'swi1',
            'swi2', 'system_start_time']
...
directory = r"H:\My Drive\P.h.D_Projects\Tillage_Mapping\Data"
# directory = r"H:\My Drive\P.h.D_Projects\Tillage_Mapping\Data"
L7raw = pd.read_csv(directory + "\L7_T1C2L2_timeSeries_2021-08-01_2022-07-31_survey_pols.csv")
L8raw = pd.read_csv(directory + "\L8_T1C2L2_timeSeries_2021-08-01_2022-07-31_survey_pols.csv")
...
# selectedCrops_df = L_8_7_raw.copy().loc[L_8_7_raw['CropType'].isin(selected_crops)]

# L_8_7_raw = selectedCrops_df
###############################################################################
# Remove swi2 
L_8_7_raw = L_8_7_raw.drop(columns=["EVI", "swi1"], axis=1).copy()

# Drop na 
L_8_7_raw.dropna(how='any', subset=['NDVI', 'blue', 'green', 'red', 'ni', 'swi2'], inplace=True)
...
indices = ['NDVI', 'blue', 'green', 'red', 'ni', 'swi2']
# indices = ["NDVI", "EVI", "blue"]
...
for indeks in indices:
  ...
  print(indeks, "done!")
print(indeks_list)
regular_dfs.to_csv(r"H:\My Drive\P.h.D_Projects\Tillage_Mapping\Data\Crop_type_TSregular_dfs_2021_2022_survey_pols.csv")


# +
# Add WSDA crop types, rename the columns and save the smoothed timeSeries dataframe
raw_df  = pd.read_csv(r"H:\My Drive\P.h.D_Projects\Tillage_Mapping\Data\L8_T1C2L2_timeSeries_2016-08-01_2019-07-31_WSDA_2018.csv")
# regular_dfs= pd.read_csv(path1, index_col=0)

raw_df[["PolygonID", "LastSurveyDate", "CropType", "Acres"]].copy()

unique_pointID_dataframe = raw_df[["PolygonID", "LastSurveyDate", "CropType", "Acres"]].copy().drop_duplicates(subset="PolygonID")
unique_pointID_dataframe.rename(columns={"PolygonID": "pointID"},inplace=True)
...
unique_pointID_dataframe.loc["pointID"] = unique_pointID_dataframe["pointID"].astype(
    int)
smoothed_df = pd.merge(regular_dfs, unique_pointID_dataframe[[
                       "pointID", "LastSurveyDate", "Acres", "CropType"]], on="pointID")

print(smoothed_df.shape)
smoothed_df.to_csv(
    r"H:\My Drive\P.h.D_Projects\Tillage_Mapping\Data\Crop_type_TS\smoothed_df_2017_2018.csv")

# -

regular_dfs = pd.read_csv(r"H:\My Drive\P.h.D_Projects\Tillage_Mapping\Data\Crop_type_TS\regular_dfs_2020_2021_survey_pols.csv")
regular_dfs.columns

# +
# Rename the columns of regular df associated with survey pols time series data 
# and save the smoothed time-series dataframe
regular_dfs = pd.read_csv(r"H:\My Drive\P.h.D_Projects\Tillage_Mapping\Data\Crop_type_TS\regular_dfs_2020_2021_survey_pols.csv", index_col=0)
regular_dfs.columns
...
regular_dfs = regular_dfs.loc[:, ~regular_dfs.columns.duplicated()]
regular_dfs["pointID"].astype(int)
smoothed_df = regular_dfs

print(smoothed_df.shape)
smoothed_df.to_csv(
    r"H:\My Drive\P.h.D_Projects\Tillage_Mapping\Data\Crop_type_TS\smoothed_df_2021_2022_survey_pols.csv")

# -

smoothed_df[["NDVI", "blue", "green", "red", "ni", "swi2"]].describe()
smoothed_df.pointID.value_counts().value_counts()
smoothed_d
