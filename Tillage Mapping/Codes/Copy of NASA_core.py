...
def regularize_a_field_annual_basis(a_df, V_idks="NDVI", interval_size=10, start_year=2008, end_year=2021):
    ...
    assert (len(a_df.pointID.unique()) == 1)
    ...
    if "dataset" in a_df.columns:
        regular_cols = ['pointID', 'dataset', 'human_system_start_time', V_idks]
    else:
        regular_cols = ['pointID', 'human_system_start_time', V_idks]

    regular_df = pd.DataFrame(data = None, 
                              index = np.arange(no_steps), 
                              columns = regular_cols)

    regular_df['pointID'] = a_df.pointID.unique()[0]
    if "dataset" in a_df.columns:
...
def create_calendar_table(SF_year):
    start = str(SF_year) + "-09-01"
    end = str(SF_year) + "-08-15"
...
def regularize_a_field(a_df, V_idks="NDVI", interval_size=10, start_year=2008, end_year=2021):
    ...
    missing_begin_df.pointID = regular_df.pointID.unique()[0]
    ...
    missing_end_df.pointID = regular_df.pointID.unique()[0]
